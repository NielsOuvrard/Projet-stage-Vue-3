<script setup lang="ts">
    ...
    import { CreditsFromMovie } from '../types/apiType';

    const route = useRoute()
    const movieCredit = ref<CreditsFromMovie[]>()
    ...
    onMounted(async () => {
        movieCredit.value = await API.allActorsFromMovieRequest(
...
<template>
    <div class="actor-cards">
        <div class="actor-cards__table">
            <div class="actor-cards__row">
                <div
                    v-for="actor in movieCredit"
                    :key="actor.id"
                    class="actor-cards__cell"
                >
                    {{ actor.name }}
                </div>
            </div>
            <div class="actor-cards__row">
                ...
                >
                    ...
                    >
                        <img
                            class="actor-cards__image"
                            :src="checkImage(actor.profile_path)"
                        />
                    </RouterLink>
                </div>
            </div>
...
<style lang="scss" scoped>
    .actor-cards {
        ...
        &__image {
            border-radius: 0.4em;
            height: 9.375em;
            box-shadow: 0.125em 0.125em 0.25em rgb(0, 0, 0);
        }
    }
</style>
